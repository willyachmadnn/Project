...
<style>
    /* Force 24-hour format globally with maximum priority */
    html {
    ...
    /* Ultra-aggressive AM/PM hiding with maximum specificity */
    ...
    *::-webkit-datetime-edit-ampm-field * {
    ...
    /* Hide any element containing AM/PM text */
    [aria-label*="AM"],
    [aria-label*="PM"],
    [title*="AM"],
    [title*="PM"],
    ...
    input[type="time"] {
        ...
        --time-format: 24 !important;
        --webkit-time-format: 24 !important;
        --moz-time-format: 24 !important;
    }
    
    /* Global 24-hour format enforcement */
    * {
        --time-format: 24 !important;
        --webkit-time-format: 24 !important;
        --moz-time-format: 24 !important;
...
<script>
// Force 24-hour format using JavaScript
document.addEventListener('DOMContentLoaded', function() {
    function force24HourFormat() {
        ...
        timeInputs.forEach(function(input) {
            ...
            input.addEventListener('focus', function() {
                setTimeout(() => {
                    // Try to find and hide AM/PM elements
            ...
            // Set step to prevent seconds
            input.step = '60';
            
            // Override showPicker to prevent AM/PM
            const originalShowPicker = input.showPicker;
            if (originalShowPicker) {
                input.showPicker = function() {
                    originalShowPicker.call(this);
                    setTimeout(() => {
                        // Try to hide AM/PM in picker
                        const picker = document.querySelector('input[type="time"]:focus');
                        if (picker && picker.shadowRoot) {
                            const ampmElements = picker.shadowRoot.querySelectorAll('[part*="ampm"], [part*="meridiem"]');
                            ampmElements.forEach(el => {
                                el.style.display = 'none !important';
                                el.remove();
                            });
    ...
    // Re-apply when modal opens (for Alpine.js)
    document.addEventListener('alpine:init', () => {
        setTimeout(force24HourFormat, 100);
    });
    ...
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            if (mutation.addedNodes.length > 0) {
                setTimeout(force24HourFormat, 10);
            }
        });
    });
